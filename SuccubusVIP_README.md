...
桌面启动时自动读取 Windows 已启用的系统代理（不改 Git 全局设置）。连接错误自动重试一次；请保持代理软件运行。修改仍保留在本地，修复连接后可重新提交。

默认窗口 1000×680，最小 920×620，顶部图片 70×64 像素；主要编辑按钮已检查在这两个尺寸下完整显示。替换源码后请关闭旧窗口并重新启动。
