...
class RosterApp:
    def __init__(self, root: tk.Tk, smoke=False):
        ...
        style.configure(".", font=("Microsoft YaHei UI", 10), background="#201929", foreground="#f3eaf8")
        ...
        style.configure("Title.TLabel", font=("Microsoft YaHei UI", 23, "bold"), foreground="#f7e9fa")
        style.configure("TButton", padding=(12, 8), background="#382745", borderwidth=0)
        ...
        self.path_label.pack(anchor="w", pady=(2, 13))
        ...
        for badge, name, filename in BADGES:
            with Image.open(ASSET_DIR / filename) as image:
                ...
                displayed.thumbnail((112, 104), Image.Resampling.LANCZOS)
            ...
            card = tk.Radiobutton(cards, text=f"{badge}  {name}", image=photo, compound="top",
                                  ...
                                  font=("Microsoft YaHei UI", 10), padx=13, pady=10, cursor="hand2")
        ...
        ttk.Label(editor, text="编辑玩家", font=("Microsoft YaHei UI", 14, "bold"), foreground="#f1d8fc").pack(anchor="w", pady=(4, 16))
        ttk.Label(editor, text="显示昵称（与游戏内完全一致）").pack(anchor="w")
        self.name_var = tk.StringVar()
        self.name_entry = ttk.Entry(editor, textvariable=self.name_var)
        self.name_entry.pack(fill="x", pady=(7, 16))
        ...
        self.badge_combo.pack(fill="x", pady=(7, 14))
        ...
        self.button(editor, "仅保存本地 TXT", self.save_local).pack(fill="x", pady=(18, 4))
        ttk.Label(editor, text="用上方徽章卡片也能选择类别。\n删除并发布后，对应玩家会失去标识。\n\n提交只更新 SuccubusList.txt，\n不会提交仓库里的其他文件。", wraplength=268, foreground="#a58db2", justify="left").pack(anchor="w", pady=(14, 0))
        self.summary_var = tk.StringVar()
        ttk.Label(outer, textvariable=self.summary_var, foreground="#c897dc").pack(anchor="w", pady=(12, 6))
        self.status_var = tk.StringVar(value="就绪")
        ttk.Label(outer, textvariable=self.status_var, wraplength=1020).pack(anchor="w")
